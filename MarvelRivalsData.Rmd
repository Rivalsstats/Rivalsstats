...
hero_stats <- hero_stats %>%
  ...
  left_join(select(all_heroes, id, name, icon, role, attack_type, difficulty), by = c("hero_data_hero_id" = "id"))
  ...
  icons <- sapply(sorted_parts, function(x) {
    paste0(
      '<div style="display:inline-block; vertical-align: middle; margin-right: 5px; background-color:',rolebg_color,'; border-radius: 4px; padding: 2px 6px;">',
        ...
        '<img src="../icons/role_icons/', x$role, '.png" height="30px" style="vertical-align: middle;">',
      '</div>'
    )
  })
    ...
    <div id="{hero_name_clean}_Card" class="card h-100 shadow mb-3 p-3 dark-card">
      ...
      <div class="card-body">
        ...
        <div id="RoleAndType" class="d-flex align-items-center mb-2">
          <img src="icons/role_icons/{row$role}.png" alt="{row$role}" class="me-2" style="width: 24px; height: 24px;">
          <span class="text-light">{row$attack_type}</span>
        </div>
        ...
        <!-- Overall Stats -->
        <p class="card-text">
          <div>Winrate: <span class="{winrate_class}">{row$win_rate}%</span></div>
          <div>Pickrate: <span class="badge bg-info">{row$pick_rate}%</span></div>
        </p>
      
        <!-- More Info Button -->
        <button class="btn btn-primary" type="button" data-bs-toggle="modal" data-bs-target="#heroModal_{hero_name_clean}" title="Show hero counters and synergies">
          More Info
        </button>
      </div>
    </div>
...
generate_modal <- function(hero_data, hero_counters, hero_synergy) {
  # Clean hero name (remove spaces) for use in IDs.
  hero_name_clean <- gsub(" ", "", hero_data$name)
  
  ## --- COUNTERS SECTION (example for "Vanguard") ----------------
  # Filter hero_counters for this hero and role.
  counters_vanguard <- subset(hero_counters, hero == hero_data$name & role_lose == "VANGUARD")
  
  if(nrow(counters_vanguard) > 0) {
    best_counters  <- head(counters_vanguard[order(-counters_vanguard$win_rate), ], 3)
    worst_counters <- head(counters_vanguard[order(counters_vanguard$win_rate), ], 3)
    all_counters   <- counters_vanguard
  } else {
    best_counters  <- worst_counters <- all_counters <- data.frame()
  }
  best_counter_html <- if(nrow(best_counters) > 0) {
    paste(apply(best_counters, 1, function(x) generate_counter_item(as.list(x))), collapse = "\n")
  } else ""
  
  worst_counter_html <- if(nrow(worst_counters) > 0) {
    paste(apply(worst_counters, 1, function(x) generate_counter_item(as.list(x))), collapse = "\n")
  } else ""
  
  all_counter_html <- if(nrow(all_counters) > 0) {
    paste(apply(all_counters, 1, function(x) generate_counter_item(as.list(x))), collapse = "\n")
  } else ""
  
  ## --- SYNERGY SECTION (example for "Vanguard") ----------------
  # Filter hero_synergy for this hero (using hero1) and a given role.
  synergies_vanguard <- subset(hero_synergy, hero1 == hero_data$name & role2 == "VANGUARD")
  
  if(nrow(synergies_vanguard) > 0) {
    best_synergies  <- head(synergies_vanguard[order(-synergies_vanguard$win_rate), ], 3)
    worst_synergies <- head(synergies_vanguard[order(synergies_vanguard$win_rate), ], 3)
    all_synergies   <- synergies_vanguard
  } else {
    best_synergies  <- worst_synergies <- all_synergies <- data.frame()
  }
  
  best_synergy_html <- if(nrow(best_synergies) > 0) {
    paste(apply(best_synergies, 1, function(x) generate_synergy_item(as.list(x))), collapse = "\n")
  } else ""
  
  worst_synergy_html <- if(nrow(worst_synergies) > 0) {
    paste(apply(worst_synergies, 1, function(x) generate_synergy_item(as.list(x))), collapse = "\n")
  } else ""
  
  all_synergy_html <- if(nrow(all_synergies) > 0) {
    paste(apply(all_synergies, 1, function(x) generate_synergy_item(as.list(x))), collapse = "\n")
  } else ""
  
  ## --- BUILD THE MODAL HTML ---------------------------------------
  modal_html <- glue('
<div class="modal fade" id="heroModal_{hero_name_clean}" tabindex="-1" aria-labelledby="heroModalLabel_{hero_name_clean}" aria-hidden="true" style="display: none;">
  <div class="modal-dialog modal-xl">
    <div class="modal-content dark-modal">
      <div class="modal-header">
        <h5 class="modal-title" id="heroModalLabel_{hero_name_clean}">{hero_data$name} Details</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      
      <div class="modal-body">
        <!-- Section 1: Opponent Data (Counters) -->
        <h5 class="mb-3">Counters</h5>
        <!-- Row 1: Opposed Vanguards-->
        <div class="row mb-4 role-section">
          <div class="col-md-4">
            <h6>Vanguards</h6>
            <strong>Best:</strong>
            <div id="BestVanguardCounters" class="d-flex gap-3 mt-1">
              {best_counter_html}
            </div>
          </div>
          ...
          <div class="col-md-4">
            <h6>&nbsp;</h6>
            <strong>Worst:</strong>
            <div id="WorstVanguardCounters" class="d-flex gap-3 mt-1">
              {worst_counter_html}
            </div>
          </div>
          ...
          <div class="col-md-4">
            ...
            <div class="d-flex hero-carousel">
              <div id="AllVanguardCounters" class="hero-carousel-track marquee">
                {all_counter_html}
              </div>
            </div>
          </div>
        ...
        <!-- Section 2: Allied Data (Synergies) -->
        <h5 class="mb-3">Synergies</h5>
        <!-- Row 1: Allied Vanguards -->
        <div class="row mb-4 role-section">
          <div class="col-md-4">
            <h6>Vanguards</h6>
            <strong>Best:</strong>
            <div id="BestVanguardSynergies" class="d-flex gap-3 mt-1">
              {best_synergy_html}
            </div>
          </div>
          ...
          <div class="col-md-4">
            <h6>&nbsp;</h6>
            <strong>Worst:</strong>
            <div id="WorstVanguardSynergies" class="d-flex gap-3 mt-1">
              {worst_synergy_html}
            </div>
          </div>
          ...
          <div class="col-md-4">
            ...
            <strong>All: </strong>
            <div class="d-flex hero-carousel">
              <div id="AllVanguardSynergies" class="hero-carousel-track marquee">
                {all_synergy_html}
              </div>
            </div>
          </div>
