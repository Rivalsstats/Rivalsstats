...
#merge and remove data created by using different api endpoints
match_players <- match_players %>%
  mutate(hero_data_playtime = coalesce(hero_data_playtime, hero_data_play_time)) %>%
  mutate(hero_data_hit_rate = coalesce(hero_data_hit_rate, hero_data_session_hit_rate)) %>%
  mutate(hero_data_playtime = round(hero_data_playtime,0)) %>%
  select(-hero_data_play_time, -hero_data_hero_icon, -hero_data_session_hit_rate)
