...
for(match_uid in unique(matches$match_uid)){
  ...
  # Updated extraction: use hero_id and hero_data_playtime directly.
  ...
  if(file.exists(file_path)) {
    ...
    if(is.null(match_data$season)) {
      match_data$season <- match_info$season
      updated <- TRUE
    }
    if(is.null(match_data$match_timestamp)) {
      match_data$match_timestamp <- match_info$match_timestamp
      updated <- TRUE
    }
    if(is.null(match_data$map)) {
      match_data$map <- all_maps$name[all_maps$id == match_info$match_map_id]
      updated <- TRUE
    }
...
hero_index <- list()
for(match_uid in names(slim_index)) {
  for(team in slim_index[[match_uid]]$teams) {
    for(hero in team$heroes) {
      hero_id <- hero$hero_id
      if(!is.na(slim_index[[match_uid]]$duration) && (hero$playtime / slim_index[[match_uid]]$duration) < 0.8) {
        next  # Skip this hero if playtime is less than 80% of the match duration.
      }
      if(match_uid == "4891597_1741915329_220_11001_10"){
        print(hero_id)
        print(hero$playtime / slim_index[[match_uid]]$duration)
      }
      # Initialize with outcome-specific vectors if not already initialized
      if(is.null(hero_index[[hero_id]])) {
         hero_index[[hero_id]] <- list(win = character(), loss = character())
      }
      # Use the outcome to append the match UID accordingly
      outcome <- team$outcome  # "win" or "loss"
