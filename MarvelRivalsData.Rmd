...
```{r debugPrints, echo=FALSE}
library('here')
file.exists(here("icons", "hero_icons", "captain_america.png"))
```
...
exclude_heroes <- c("The Punisher")  # Excluded heroes
...
conditional_teams <- match_players %>%
  ...
  mutate(
    ...
    has_excluded_hero = map_lgl(team, ~ any(.x$play[.x$name %in% exclude_heroes] >= play_threshold, na.rm = TRUE))
  )
