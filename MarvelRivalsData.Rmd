...
hero_match <- match_players %>%
  group_by(match_uid, name.y, is_win) %>%
  summarise(
    # Since a hero can only be on one team per match, 
    # just take the win status from the first row.
    hero_win = as.numeric(first(is_win)),
    .groups = "drop"
  )

# Step 2: Join with the opponent team composition.
hero_vs_opponent_comp <- hero_match %>%
  rename(hero = name.y) %>%
  inner_join(
    team_roles_discrete %>% 
  ...
  # Ensure we compare the hero's team to the opponent's team.
