...
all_heroes <- fromJSON("data/latest/latest_heroes.json")
# Read the JSON file and flatten nested elements
maps_data <- fromJSON("data/latest/latest_maps.json", flatten = TRUE)

# Extract the 'maps' list which contains each map as an element
all_maps <- maps_data$maps
...
competitive_matches <- matches[tolower(matches$gamemode) == "competitive", ]
...
for (i in 1:nrow(all_heroes)) {
  hero_name <- gsub(" ", "_", str_to_lower(all_heroes$name[i]))  # Replace spaces with underscores
  file_path <- paste0("icons/hero_icons/", str_to_lower(hero_name), ".png")

  if (!file.exists(file_path)) {  # Only download if it doesn't exist
    file_path <- "favicon/web-app-manifest-512x512.png"
  }
...
# If the column doesn't exist, add it as NA.
if (!"hero_data_playtime" %in% names(match_players)) {
  match_players$hero_data_playtime <- NA
}
if (!"hero_data_hit_rate" %in% names(match_players)) {
  match_players$hero_data_hit_rate <- NA
}

match_players <- match_players %>%
  mutate(
    hero_data_playtime = coalesce(hero_data_playtime, hero_data_play_time),
    hero_data_hit_rate = coalesce(hero_data_hit_rate, hero_data_session_hit_rate),
    hero_data_playtime = round(hero_data_playtime, 0)
  ) %>%
  select(-any_of(c("hero_data_play_time", "hero_data_hero_icon", "hero_data_session_hit_rate")))
...
hero_stats <- hero_stats %>%
  ...
  left_join(select(all_heroes, id, name, icon, role, attack_type, difficulty, bio), by = c("hero_data_hero_id" = "id"))

#fix non existing icons
hero_stats <- hero_stats %>%
  mutate(
    # Create a flag for rows where the icon is missing
    icon_is_na = icon == "icons/hero_icons/NA.png",
    
    # Replace the icon for those rows
    icon = if_else(icon_is_na, 
                   "favicon/web-app-manifest-512x512.png", 
                   icon),
    
    # Set the additional columns to "UNKNOWN" if icon was missing, otherwise keep current values
    name = if_else(icon_is_na, "UNKNOWN", name),
    role = if_else(icon_is_na, "UNKNOWN", role),
    bio = if_else(icon_is_na, "UNKNOWN", bio),
    attack_type = if_else(icon_is_na, "UNKNOWN", attack_type),
    
    # Set difficulty to 0 if the icon was missing, otherwise keep the current value
    difficulty = if_else(icon_is_na, "0", difficulty)
  ) %>%
  select(-icon_is_na)  # Remove the temporary flag column if no longer needed
...
get_role_count <- function(comp_string, role_name) {
  # Check if role_name is missing, NULL, NA, or an empty string.
  if (missing(role_name) || is.null(role_name) || is.na(role_name) || role_name == "") {
    return(0)
  }
  
  # (Optional) Also check for comp_string issues.
  if (missing(comp_string) || is.null(comp_string) || is.na(comp_string) || comp_string == "") {
    return(0)
  }
  ...
  for (p in parts) {
    ...
    if (length(tokens) >= 2 && !is.na(tokens[2]) && tolower(tokens[2]) == tolower(role_name)) {
...
winning_plays <- all_plays %>%
  ...
  # bring in both competitiveness and the team's total performance (win_TRUE)
  inner_join(
    match_team_perf %>% select(match_uid, win_TRUE, competitiveness),
    by = "match_uid"
  ) %>%
  
  # for each match, compute how many players are on the team
  group_by(match_uid) %>%
  mutate(
    team_size     = n(),
    avg_team_perf = win_TRUE / team_size,           # average performance of that team
    rel_perf      = performance / avg_team_perf,     # how you did versus your teammates
    mvp_flag      = if_else(player_uid == mvp, 1, 0) # 1 if you were MVP, else 0
  ) %>%
  ungroup() %>%
  
  # now fold in playtime, competitiveness, AND the MVP-relative boost
  mutate(
    performance_adjusted = performance
      * (total_playtime / median(total_playtime, na.rm = TRUE))
      * competitiveness
      * if_else(mvp_flag == 1, rel_perf, 1)
  )


# 4. Identify "consistent players": for each player and hero, require at least 10 wins 
...
generate_stars <- function(difficulty) {
  # Coerce the difficulty to numeric
  diff_val <- as.numeric(difficulty)
  
  # If the value is NA, then treat it as 0
  if (is.na(diff_val)) diff_val <- 0
  
  # Clamp the value between 0 and 5 and round to an integer
  diff_val <- round(pmax(0, pmin(diff_val, 5)))
  
  filled <- paste(rep('<i class="fas fa-star"></i>', diff_val), collapse = "")
  empty  <- paste(rep('<i class="far fa-star"></i>', 5 - diff_val), collapse = "")
...
target_hero <- "1023"
defensive_ult_set <- c("1031", "1050", "1020", "1025", "1016")
play_threshold <- 0.5  # 50% threshold for playtime
exclude_heroes <- c("1014")  # Excluded heroes
...
teams <- match_players %>%
  group_by(match_uid, is_win) %>%
  summarise(team = list(data.frame(name = name.y, play = playtime_percentage, hero_data_hero_id=hero_data_hero_id)), .groups = "drop")

# Compute team-level metrics:
teams <- teams %>%
  mutate(
    # Does the team have Rocket Raccoon?
    rocket_present = map_lgl(team, ~ target_hero %in% .x$hero_data_hero_id),
    ...
    support_count = map_int(team, ~ {
      df <- .x
      sum(df$hero_data_hero_id %in% defensive_ult_set & df$hero_data_hero_id != target_hero & df$play >= play_threshold)
    }),
    # For Rocket, take the maximum playtime if he appears multiple times.
    rocket_play = map_dbl(team, ~ {
      df <- .x
      rp <- df$play[df$hero_data_hero_id == target_hero]
...
# ----- STEP 1: Build Teams Directly from match_players -----
conditional_teams <- match_players %>%
  group_by(match_uid, is_win) %>%
  summarise(team = list(data.frame(name = name.y, play = playtime_percentage,hero_data_hero_id=hero_data_hero_id)), .groups = "drop") %>%
  mutate(
    # Does the team have Rocket Raccoon?
    ...
    has_excluded_hero = map_lgl(team, ~ any(.x$play[.x$hero_data_hero_id %in% exclude_heroes] >= play_threshold, na.rm = TRUE))
  )

# ----- STEP 2: Separate Teams with and without Rocket -----
rocket_teams <- conditional_teams %>% filter(rocket_present)
opp_teams <- conditional_teams %>% filter(!rocket_present)

# ----- STEP 3: Pair Opposing Teams by Match ID -----
conditional_matched_teams <- inner_join(rocket_teams, opp_teams, by = "match_uid", 
                            suffix = c("_rr", "_opp"))

# ----- STEP 4: Apply Filtering Criteria -----
filtered_matches_no_punisher <- conditional_matched_teams %>%
  filter(
    # On Rocket's team: Rocket must have played at least 50%
    rocket_play_rr >= play_threshold,
...
# ----- STEP 5: Compute Rocket Raccoon's Win Rate -----
rocket_win_rate_punisher <- mean(filtered_matches_no_punisher$is_win_rr)
rocket_games_no_punisher <- sum(filtered_matches_no_punisher$rocket_play_rr)

filtered_matches_with_punisher <- conditional_matched_teams %>%
  filter(
    # On Rocket's team: Rocket must have played at least 50%
    rocket_play_rr >= play_threshold,
    # Exclude teams that contain Punisher or Winter Soldier.
...
rocket_games_punisher <- sum(filtered_matches_with_punisher$rocket_play_rr)
...
# Replace empty vectors with 0 for consistency
rocket_win_rate <- if(length(rocket_win_rate) == 0) 0 else rocket_win_rate
rocket_overall_winrate <- if(length(rocket_overall_winrate) == 0) 0 else rocket_overall_winrate
rocket_win_rate_with_punisher <- if(length(rocket_win_rate_with_punisher) == 0) 0 else rocket_win_rate_with_punisher
rocket_win_rate_punisher <- if(length(rocket_win_rate_punisher) == 0) 0 else rocket_win_rate_punisher
...
winrate_df <- data.frame(
  ...
  WinRate = c(
    ifelse(is.finite(rocket_win_rate), round(rocket_win_rate * 100, 1), 0), 
    ifelse(is.finite(rocket_overall_winrate), round(rocket_overall_winrate, 1), 0),
    ifelse(is.finite(rocket_win_rate_with_punisher), round(rocket_win_rate_with_punisher * 100, 1), 0), 
    ifelse(is.finite(rocket_win_rate_punisher), round(rocket_win_rate_punisher * 100, 1), 0)
  )
...
```{r createIndex, echo=TRUE, message=FALSE, warning=FALSE}
# Initialize slim index list (one entry per match)
slim_index <- list()

players <- match_players %>%
  mutate(match_uid = as.character(match_uid),
         player_uid = as.character(player_uid),
         hero_id = as.character(hero_id))

extract_hero_info <- function(player_row) {
  list(
    hero_id = as.character(player_row$hero_id),
    playtime = player_row$hero_data_playtime
  )
}

asssure_match_data <- function(match_uid,match_info){
  file_path <- file.path("data", "matches", paste0(match_uid, ".json"))
  if(file.exists(file_path)) {
    # Check if file is not empty
    if(file.info(file_path)$size == 0) {
      message("Skipping file (empty): ", file_path)
      return(NULL)
    }
    
    # Attempt to load the JSON file with error handling
    match_data <- tryCatch({
      fromJSON(file_path)
    }, error = function(e) {
      message("Error reading JSON from file: ", file_path)
      message("Error details: ", e$message)
      return(NULL)
    })
    
    # If match_data is NULL, exit the function
    if(is.null(match_data)) {
      message("Skipping match_uid ", match_uid, " due to JSON read error.")
      return(NULL)
    }
    current_match_players <- match_players[match_players$match_uid == match_uid, ]
    
    if("playtime_percentage" %in% colnames(current_match_players) &&
       "hero_data_playtime" %in% colnames(match_players)){
      durations <- current_match_players %>%
        filter(playtime_percentage > 0) %>%
        mutate(estimated_duration = hero_data_playtime / playtime_percentage) %>%
        pull(estimated_duration)
      match_duration <- if(length(durations) > 0) max(durations, na.rm = TRUE) else NA
    } else {
      match_duration <- NA
    }
    
    # Flag to track if we updated any field
    updated <- FALSE
    
    # Check and add missing fields from slim_index
    if(is.null(match_data$season) || !identical(match_data$season, match_info$season)) {
      match_data$season <- match_info$season
      updated <- TRUE
    }
    if(is.null(match_data$match_timestamp) || !identical(match_data$match_timestamp, match_info$match_timestamp)) {
      match_data$match_timestamp <- match_info$match_timestamp
      updated <- TRUE
    }
    expected_map <- all_maps$name[all_maps$id == match_info$match_map_id]
    if(is.null(match_data$map) || !identical(match_data$map, expected_map)) {
      match_data$map <- expected_map
      updated <- TRUE
    }
    if(is.null(match_data$duration) || !isTRUE(all.equal(match_data$duration, match_duration, tolerance = 1e-3))) {
      match_data$duration <- match_duration
      updated <- TRUE
    }
    
    # Write the updated data back if any changes were made
    if(updated) {
      # Use tryCatch to catch errors during write as well
      tryCatch({
        write(toJSON(match_data, auto_unbox = TRUE, pretty = TRUE), file = file_path)
      }, error = function(e) {
        message("Error writing JSON to file: ", file_path)
        message("Error details: ", e$message)
      })
    }
  }
}
map_index <- list()  # Initialize map index 
# Loop over each match to build the minimal index
for(i in seq_len(nrow(matches))){
  # Get match metadata row (assuming one row per match)
  match_info <- matches[i, ]
  match_uid <- match_info$match_uid
  # Filter player data for this match
  current_match_players <- players %>% filter(match_uid == !!match_uid)
  
  # --- Compute match duration ---
  # Using playtime_percentage (0-1) and hero_data_playtime to estimate duration
  if("playtime_percentage" %in% colnames(current_match_players) && "hero_data_playtime" %in% colnames(match_players)){
    durations <- current_match_players %>%
      filter(playtime_percentage > 0) %>%
      mutate(estimated_duration = hero_data_playtime / playtime_percentage) %>%
      pull(estimated_duration)
  ...
  # --- Build teams facets ---
  # Separate players by outcome (win vs loss)
  winning_players <- current_match_players %>% filter(is_win == TRUE)
  losing_players <- current_match_players %>% filter(is_win == FALSE)
  
  # Build minimal heroes list for each outcome
  winning_heroes <- if(nrow(winning_players) > 0) {
    lapply(seq_len(nrow(winning_players)), function(i) {
      extract_hero_info(winning_players[i, ])
    })
  } else {
    list()
  }
  
  losing_heroes <- if(nrow(losing_players) > 0) {
    lapply(seq_len(nrow(losing_players)), function(i) {
      extract_hero_info(losing_players[i, ])
    })
  } else {
    list()
  }
  
  # Build teams structure: one for winners and one for losers.
  teams <- list(
    list(outcome = "win", heroes = winning_heroes),
    list(outcome = "loss", heroes = losing_heroes)
  )
  # Create a slim entry for the match with only the filtering facets.
  slim_index[[match_uid]] <- list(
    match_uid = match_uid,
    season = match_info$season,
    match_timestamp = match_info$match_timestamp,
    gamemode = paste0(toupper(substr(match_info$gamemode, 1, 1)),
                    substr(match_info$gamemode, 2, nchar(match_info$gamemode))),
    map = all_maps$name[all_maps$id == match_info$match_map_id],
    duration = match_duration,
    teams = teams
  )
  #built map index
  map_id <- as.character(match_info$match_map_id)
  if (is.null(map_index[[map_id]])) {
    map_index[[map_id]] <- c(match_uid)
  } else {
    map_index[[map_id]] <- c(map_index[[map_id]], match_uid)
  }
  asssure_match_data(match_uid,match_info)
}

# --- Build an inverted index for heroes by team outcome ---
hero_index <- list()
for(i in seq_len(nrow(match_players))) {
  player <- match_players[i, ]
  
  # Skip this player if playtime_percentage is below 80%
  if(!is.na(player$playtime_percentage) && player$playtime_percentage < 0.8) {
    next
  }
  
  hero_id <- as.character(player$hero_data_hero_id)
  match_uid <- as.character(player$match_uid)
  outcome <- if(player$is_win) "win" else "loss"
  
  if(is.null(hero_index[[hero_id]])) {
    hero_index[[hero_id]] <- list(win = character(), loss = character())
  }
  
  hero_index[[hero_id]][[outcome]] <- unique(c(hero_index[[hero_id]][[outcome]], match_uid))
}


all_match_uids <- names(slim_index)

sorted_match_uids <- all_match_uids[order(sapply(slim_index, function(x) x$match_timestamp), decreasing = TRUE)]

# Select a preview of 50 matches (for example, the most recent ones)
preview_uids <- head(sorted_match_uids, 50)
preview_index <- slim_index[preview_uids]

preview_index <- lapply(slim_index[preview_uids], function(match) {
  # Return only the elements except 'teams'
  match[setdiff(names(match), "teams")]
})


# For all other matches, just save their match_uids
other_uids <- setdiff(all_match_uids, preview_uids)

```

```{r exportIndex, echo=TRUE}
# Define the output file path for your JSON index
output_file <- "data/index.json"

# Convert the index to JSON
json_index <- toJSON(preview_index, pretty = TRUE, auto_unbox = TRUE)

# Write the JSON file to disk
write(json_index, file = output_file)

cat("Index exported to:", output_file)
```

```{r exportHeroIndex, echo=TRUE}
# Define the output file path for your JSON index
output_file <- "data/heroes_index.json"

# Convert the index to JSON
json_index <- toJSON(hero_index, pretty = TRUE, auto_unbox = TRUE)
...
```{r exportMatchIndex, echo=TRUE}
# Define the output file path for your JSON index
output_file <- "data/match_index.json"

# Convert the index to JSON
json_index <- toJSON(other_uids, pretty = TRUE, auto_unbox = TRUE)
...
```{r exportMapIndex, echo=TRUE}
# Define the output file path for your JSON index
output_file <- "data/map_index.json"

# Convert the index to JSON
json_index <- toJSON(map_index, pretty = TRUE, auto_unbox = TRUE)
...
data <- list(
  ...
  rocket_win_rate_one_defensive = round(rocket_win_rate*100,2),
  rocket_win_rate_no_punisher = round(rocket_win_rate_punisher*100,2),
  rocket_win_rate_with_punisher = round(rocket_win_rate_with_punisher*100,2)
)
