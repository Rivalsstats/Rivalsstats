...
hero_counters_table <- hero_counters_table %>%
  mutate(icon_win = paste0(
    '<div style="position: relative; display: inline-block;">',
      ...
      '<div style="position: absolute; bottom: 6px; left: 3px; background: rgba(0, 0, 0, 0.6); border-radius: 4px; padding: 2px 4px; height: 22px; display: flex; align-items: center;">',
...
datatable(hero_synergy_table, escape = FALSE, options = list(pageLength = 10, autoWidth = TRUE, columnDefs = list(
    list(targets = c(1,2), visible = FALSE)  
  ))) %>%
  formatStyle(
...
parse_composition <- function(comp_string) {
  ...
  icons <- sapply(sorted_parts, function(x) {
    paste0(
      '<div style="display:inline-block; vertical-align: middle; margin-right: 5px; background-color:#444; border-radius: 4px; padding: 2px 6px;">',
